...
def agent_node(state) -> dict: # agent node always gets state object
    ...
    llm_with_tools = llm.bind_tools(
        [get_formatted_item_context, get_formatted_reviews_context, FinalResponse], # we can also return a final response from the model instead of going through the graph again (this is useful for when we want to return a final answer to the user instead of going through the graph again)
        tool_choice="required" # auto means llm can suggest or not suggest the tool (any forces the llm to suggest any of the tools provided in the toolset)
    )
...
def intent_router_node(state) -> dict:
    ...
    conversation = []
    conversation.append(convert_to_openai_messages(messages)[-1]) # we only want to pass the last message to the intent router to decide what to do next, we dont want to pass the entire conversation history (we can experiment with this later and see if it improves performance)
    ...
    current_run  = get_current_run_tree()
    # here we get access to the first placewhere we can get the trace id
    # this one runs first and always
